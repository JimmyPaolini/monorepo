...
### 1. Create a Backup Branch (Crucial)
...
### 3. Analyze Target Branch Purpose
...
### 4. Initiate Merge and Resolve Conflicts
...
- **CRITICAL:** Do NOT commit the changes at the end. Stop after staging the resolved files.

### 5. Summarize Resolutions

Once all conflicts are resolved and staged, **do not commit**. Provide the user with a comprehensive summary containing:
...
4. **Resolution Reasoning:** How each conflict was resolved and _why_ that specific resolution was chosen to satisfy the intent of both branches.
5. **Next Steps:** Inform the user that the conflicts are resolved and staged, and they can now review and commit the changes themselves.
