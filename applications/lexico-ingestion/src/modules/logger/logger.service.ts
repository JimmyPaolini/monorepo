...
export class LexicoIngestionLogger extends ConsoleLogger {
  private static readonly isProduction =
    process.env["NODE_ENV"] === "production";

  private static readonly root = pino(
    LexicoIngestionLogger.isProduction
  ...
  private child: pino.Logger = LexicoIngestionLogger.root;

  /** Sets the context label included in every subsequent log line. */
  override setContext(context: string): void {
    super.setContext(context);
    this.child = LexicoIngestionLogger.root.child({ context });
  }
