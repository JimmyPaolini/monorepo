...
import { Entry, Translation, type PartOfSpeech } from '@monorepo/lexico-entities';
...
export class IngesterService {
  ...
  async parseEntries(wiktionaryEntry: WiktionaryEntry): Promise<Entry[]> {
    ...
    for (const heading of posHeadings.toArray()) {
      ...
      entry.translations = translationTexts.map((t) => new Translation(t, entry));
      entries.push(entry);
      index++;
    }
  ...
  private isPartOfSpeech(text: string): boolean {
    return [
      ...
      'conjunction', 'interjection', 'participle', 'numeral', 'prefix',
      'suffix', 'phrase', 'proverb', 'idiom',
