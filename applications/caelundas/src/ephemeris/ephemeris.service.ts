...
/**
 * Converts a Coordinated Universal Time date to Julian day values for both
 * ephemeris time and universal time.
 *
 * @param date - Coordinated Universal Time date to convert
...
/**
 * Returns the Swiss Ephemeris body ID for a non-node body (planet or asteroid).
 *
 * @throws When the body has no known Swiss Ephemeris constant (e.g., comets)
 */
function getSwissEphemerisConstantForBody(body: Exclude<Body, Node>): number {
  const planetConst = (
...
/**
 * Computes ecliptic longitude and latitude for a lunar node or apside.
 *
 * @remarks
 * Nodes lie on the ecliptic by definition, so latitude is always 0.
 * - North lunar node: true node longitude (direct)
 * - South lunar node: true node longitude + 180°
 * - Lunar apogee: osculating apogee longitude (osculating Black Moon Lilith)
 * - Lunar perigee: nod_aps_ut().perihelion longitude
 */
function computeNodeCoordinate(
  node: Node,
  julianDayEphemerisTime: number,
  julianDayUniversalTime: number,
  ...
  if (swissEphemerisConstant === null) {
    throw new Error(`No Swiss Ephemeris constant configured for node: ${node}`);
  }
...
/**
 * Computes ecliptic longitude and latitude for a planet or asteroid.
 */
function computeBodyCoordinate(
  body: Exclude<Body, Node>,
  julianDayEphemerisTime: number,
): { longitude: number; latitude: number } {
  const swissEphemerisConstant = getSwissEphemerisConstantForBody(body);
  const result = calc(
    julianDayEphemerisTime,
    swissEphemerisConstant,
    SWISS_EPHEMERIS_FLAGS,
...
export function getCoordinateEphemerisByBody(args: {
  ...
  for (const body of bodies) {
    ...
    for (const date of generateMinutes(start, end)) {
      const { julianDayEphemerisTime, julianDayUniversalTime } =
        dateToJulianDays(date);
      const timestamp = date.toISOString();
      const coord = isNode(body)
        ? computeNodeCoordinate(
            body,
            julianDayEphemerisTime,
            julianDayUniversalTime,
          )
        : computeBodyCoordinate(body, julianDayEphemerisTime);
...
// #region Azimuth/Elevation Ephemeris
...
export function getIlluminationEphemerisByBody(args: {
  ...
  for (const body of bodies) {
    ...
    for (const date of generateMinutes(start, end)) {
      const { julianDayUniversalTime } = dateToJulianDays(date);
      const timestamp = date.toISOString();
...
export function getDistanceEphemerisByBody(args: {
  ...
  for (const body of bodies) {
    ...
    for (const date of generateMinutes(start, end)) {
      const { julianDayEphemerisTime } = dateToJulianDays(date);
      const timestamp = date.toISOString();
...
// #region Unified Ephemeris Computation
 ...
 */
export function computeAllEphemerides(args: {
  ...
  for (const body of coordinateBodies) {
    ...
    if (isNode(body)) {
      ...
      for (const date of generateMinutes(start, end)) {
        const { julianDayEphemerisTime, julianDayUniversalTime } =
          dateToJulianDays(date);
        const coord = computeNodeCoordinate(
          body,
          julianDayEphemerisTime,
          julianDayUniversalTime,
        );
