...
describe(validateConformance, () => {
  describe("heavily modified service integration", () => {
    ...
    it.each(MOCK_SERVICES)(
      "$camel service conforms to the service template structure",
      ({ camel, content, pascal }) => {
        ...
        expect(structuralErrors).toStrictEqual([]);
      },
    );
  });
  ...
  describe(validateInstancesDirectory, () => {
    ...
    it("returns one result per subdirectory", () => {
      ...
      fs.writeFileSync(
        path.join(alphaDir, "alpha.config.ts"),
        "// 🔧 Config\n",
      );
    ...
    it("all passing files return empty error arrays", () => {
      ...
      for (const result of results) {
        for (const fileResult of result.results) {
          expect(fileResult.errors).toStrictEqual([]);
        }
      }
    });
    ...
    it("reports missing file error when instance file is absent", () => {
      ...
      const firstAlphaErrorMessage =
        alphaResult?.results[0]?.errors[0]?.message ?? "";

      expect(firstAlphaErrorMessage).toContain("Missing file:");
    });
  });
  ...
  describe(validateInstanceDirectory, () => {
    ...
    it("resolves __fieldName__ tokens in template filename to instance filename", () => {
      ...
      expect(result.results[0]?.errors).toStrictEqual([]);
    });
    ...
    it("reports missing file error when instance file is absent", () => {
      ...
      const firstErrorMessage = result.results[0]?.errors[0]?.message ?? "";

      expect(firstErrorMessage).toContain("Missing file:");
    });
  });
