...
export function validateMarkdownConformance(args: {
  ...
  const templateChildren = filterMdastNodes(templateAst.children);
  const instanceChildren = filterMdastNodes(instanceAst.children);

  const errors = validateMdastChildren(templateChildren, instanceChildren);

  return { errors };
}

/** Filters root children to only include supported mdast node types. */
function filterMdastNodes(children: unknown[]): MdastNode[] {
...
function buildErrorPositions(
  ...
  const instanceLine = buildInstanceLine(instanceHint);
...
/** Calculates the instance line number based on the hint node's end position. */
...
function matchByText(template: MdastNode, instance: MdastNode): boolean {
  return textsMatch(toString(template), toString(instance));
}
