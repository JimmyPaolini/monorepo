...
import type { MdastNode } from "./nodes";
...
describe("validateMarkdownConformance synthetic branches", () => {
  ...
  it("uses fallback error message for unknown node types", async () => {
    const parseMock = vi
  ...
  it("covers uncommon markdown message builders", async () => {
  ...
  it("derives instance insertion line when a later leaf node is missing", async () => {
    const templateRoot = createRoot([
      createPositionedNode({ type: "inlineCode", value: "present" }, 1),
      createPositionedNode({ type: "inlineCode", value: "missing" }, 2),
    ]);

    const instanceRoot = createRoot([
      createPositionedNode({ type: "inlineCode", value: "present" }, 8),
    ]);
    ...
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0]?.message).toBe("Expected inline code: `missing`");
    expect(result.errors[0]?.instanceLine).toBe(9);
  ...
  it("covers additional by-text matcher branches for supported node types", async () => {
    const templateRoot = createRoot([
      createPositionedNode({ type: "strong", value: "bold" }, 1),
      createPositionedNode({ type: "math", value: "x^2" }, 2),
      createPositionedNode({ type: "yaml", value: "key: value" }, 3),
      createPositionedNode({ type: "imageReference", value: "logo" }, 4),
      createPositionedNode({ type: "linkReference", value: "guide" }, 5),
      createPositionedNode({ type: "break", value: "" }, 6),
      createPositionedNode({ type: "footnoteReference", value: "" }, 7),
    ]);

    const instanceRoot = createRoot([
      { type: "strong", value: "bold" },
      { type: "math", value: "x^2" },
      { type: "yaml", value: "key: value" },
      { type: "imageReference", value: "logo" },
      { type: "linkReference", value: "guide" },
      { type: "break", value: "" },
      { type: "footnoteReference", value: "" },
    ]);
  ...
  it("covers every error message builder path via synthetic markdown nodes", async () => {
    const templateRoot = createRoot([
