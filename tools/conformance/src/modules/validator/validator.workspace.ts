...
export function readWorkspaceProjects(): WorkspaceProject[] {
  return DEFAULT_PROJECT_DIRECTORIES.flatMap((directoryPath) => {
    ...
    return fs
      ...
      .map((projectRootPath) => readWorkspaceProject(projectRootPath));
  });
}

/** Resolves a project by its name from the workspace project list. */
export function resolveProjectByName(args: {
  ...
  const workspaceProject = allWorkspaceProjects.find((candidateProject) => {
    return resolveProjectName(candidateProject.rootPath) === projectName;
  });
...
export function resolveProjectName(projectRootPath: string): string {
...
/** Resolves requested project names or defaults to application/component-tagged projects. */
export function resolveSelectedProjectNames(args: {
  allWorkspaceProjects: WorkspaceProject[];
  ...
  const defaultProjectNames = allWorkspaceProjects
    ...
    .map((workspaceProject) => resolveProjectName(workspaceProject.rootPath))
    .toSorted();
...
/** Parses and returns workspace project metadata from a project root path. */
function readWorkspaceProject(projectRootPath: string): WorkspaceProject {
  const projectConfigurationPath = path.join(projectRootPath, "project.json");
  try {
    const projectConfiguration = JSON.parse(
      fs.readFileSync(projectConfigurationPath, "utf8"),
