...
| TASK-039 | Update `lint-staged.config.ts` to add a CSS file pattern: `"*.css": (files) => { const relativePaths = files.map((file) => relative(process.cwd(), file)).join(","); return [\`nx affected --target=stylelint --files=\${relativePaths}\`]; }`                                                                                                                                                                                     |           |      |
...
- **FILE-015**: `lint-staged.config.ts` — Modified. Add `*.css` pattern for Stylelint pre-commit integration
